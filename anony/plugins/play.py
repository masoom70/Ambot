...
async def play_hndlr(
    ...
    force = True if "force" in _cmd else force
    video = True if _cmd.startswith("v") else video
    ...
    if not file:
        _err = m.lang["play_not_found"].format(config.SUPPORT_CHAT) if len(m.command) >= 2 else m.lang["play_usage"]
        return await sent.edit_text(_err)
    ...
    if force:
        ...
        if position != 0 or await db.get_call(m.chat.id):
            ...
            if tracks:
                ...
                await app.send_message(m.chat.id, text=m.lang["playlist_queued"].format(len(tracks)) + added)
            return
